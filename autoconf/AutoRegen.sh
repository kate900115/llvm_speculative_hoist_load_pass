...
cwd=`pwd`
if test -d /opt/llvm-source/autoconf/m4 ; then
  cd /opt/llvm-source/autoconf/m4
  llvm_src_root=/opt/llvm-source/
  llvm_obj_root=/opt/llvm/
  cd $cwd
elif test -d /opt/llvm/autoconf/m4 ; then
  cd /opt/llvm/autoconf/m4
  llvm_src_root=/opt/llvm-source/
  llvm_obj_root=/opt/llvm/
  cd $cwd
else
